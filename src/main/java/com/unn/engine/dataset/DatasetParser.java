		    ...
		    while ((line = br.readLine()) != null) { 	
		    	...
		    	if (n == 0) {
		    		...
		    		for (String bopName : cols) {
		    			...
		    			Raw bop = new Raw();
			    		bop.setDescriptor(new FunctionDescriptor(".", bopName, k));
			    		localArgs[k] = bop;
					...
					int k = -1;
					ArrayList<ValueTimeReward> inputs = new ArrayList<ValueTimeReward>();
		    		...
		    		for (String bopValStr : cols) {
	    				...
	    				inputs.add(new ValueTimeReward(localArgs[k], mappedVal, time, mappedVal));

		    			if (isReward) {
		    				for (ValueTimeReward vtr : inputs) {
		    		...
		    		if (isReward && inputs.size() == cols.length - 2) {
		    			for (ValueTimeReward input : inputs) {
		    				dataset.add(input);
		    			}
		    		}
				}
