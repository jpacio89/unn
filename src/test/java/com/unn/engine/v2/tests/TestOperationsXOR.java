...
public class TestOperationsXOR 
{
	...
	private double calculateAcc(ArrayList<Triplet> dataset, Threshold op) throws Exception {
		int correct = 0;
		op.recycle();
	...
	{		
		...
		x.setDescriptor(new FunctionDescriptor(".", ".x", 0));
		y.setDescriptor(new FunctionDescriptor(".", ".y", 1));
		
		OperationPool pool = new OperationPool(x, y);
		
		for(int i = 0; i < 1000; ++i) {
			Threshold t = pool.next();
		...
		Threshold bestOp = (Threshold) best.first();
